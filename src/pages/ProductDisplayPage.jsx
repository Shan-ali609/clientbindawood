...
const ProductDisplayPage = () => {
  ...
  setCurrentImageIndex(prev =>
    prev < data.image.length - 1 ? prev + 1 : 0 // Loop to start
  )
}

const handleScrollLeft = () => {
  setCurrentImageIndex(prev =>
    prev > 0 ? prev - 1 : data.image.length - 1 // Loop to end
  )
}
            ...
            <div className='flex items-center justify-center gap-3 my-2'>
              {
                data.image.map((img,index)=>{
                  return(
                    <div key={img+index+"point"} className={`bg-slate-200 w-3 h-3 lg:w-5 lg:h-5 rounded-full ${currentImageIndex === index   && "bg-slate-300"}`}></div>
                  )
                })
              }
            </div>
